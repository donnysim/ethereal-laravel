...
class Clipboard
{
     ...
     * @return \Illuminate\Database\Eloquent\Collection
     */
    public function getAuthorityMap(Model $authority)
    {
        $authorityMorph = $authority->getMorphClass();
        $authorityKey = $authority->getKey();
        ...
        $abilitiesTable = Helper::abilitiesTable();
        $rolesMorph = Helper::rolesModel()->getMorphClass();
        $rolesClass = Helper::rolesModelClass();

        /** @var Collection $roles */
        $roles = $rolesClass::where('entity_id', $authorityKey)
            ->where('entity_type', $authorityMorph)->get(['role_id']);
        ...
        return Helper::abilityModel()
            ...
            // Apply roles constraints
            ->orWhere(function ($query) use ($permissionsTable, $authorityKey, $authorityMorph, $rolesMorph, $roles) {
                $query->whereIn("{$permissionsTable}.entity_id", $roles->pluck('role_id'))->where("{$permissionsTable}.entity_type", $rolesMorph);
            })->get(['abilities.*', "{$permissionsTable}.forbidden"]);
    }
